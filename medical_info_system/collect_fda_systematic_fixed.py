...
def test_api_queries():
    ...
    # 测试1: 基本癌症查询
    params1 = {'search': 'cancer', 'limit': 2}
    r1 = requests.get(url, params=params1, timeout=30)
    print(f"测试1 - 基本'cancer'查询: {r1.status_code}, 总数: {r1.json().get('meta', {}).get('results', {}).get('total', 0) if r1.status_code == 200 else 'N/A'}")

    # 测试2: 带日期范围
    params2 = {'search': 'cancer AND submissions.submission_status_date:[20200101 TO 20251231]', 'limit': 2}
    r2 = requests.get(url, params=params2, timeout=30)
    print(f"测试2 - 带日期范围: {r2.status_code}, 总数: {r2.json().get('meta', {}).get('results', {}).get('total', 0) if r2.status_code == 200 else 'N/A'}")

    # 测试3: 免疫检查点关键词
    params3 = {'search': 'pd-1 OR pd-l1 OR ctla-4', 'limit': 2}
    r3 = requests.get(url, params=params3, timeout=30)
    print(f"测试3 - 免疫检查点: {r3.status_code}, 总数: {r3.json().get('meta', {}).get('results', {}).get('total', 0) if r3.status_code == 200 else 'N/A'}")
...
def collect_fda_anticancer_drugs_comprehensive():
    ...
    采集策略：
    1. 分时期采集，使用正确的API字段路径
    2. 使用多种关键词组合确保召回率
    3. 严格的去重和过滤
    """

    print("=" * 100)
    print("开始系统性FDA抗肿瘤药物采集 (修复版)")
    ...
    for period in collection_periods:
        ...
        skip = 0
        batch_size = 100
        max_records_per_period = 500

        while period_collected < max_records_per_period:
            try:
                # 构建查询 - 使用正确的字段路径
                query_parts = period['keywords'][:15]
    ...
    print(f"\nFDA药物年份分布:")
    for row in date_distribution[:15]:
    ...
    print(f"\nFDA不同药物数量: {distinct_drugs[0]['COUNT(DISTINCT drug_name_en)'] if distinct_drugs else 0}")
