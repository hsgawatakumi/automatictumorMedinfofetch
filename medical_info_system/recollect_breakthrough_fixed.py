...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('data/cde_recollection_fixed.log', encoding='utf-8'),
...
def get_drug_indication(page: Page, list_type: str, row_idx: int, table_index: int) -> str:
    """从弹窗中获取适应症"""
    indication = ""
...
def collect_breakthrough_drugs(target_count=250):
    ...
    with sync_playwright() as p:
        ...
        try:
                    ...
                    for (let i = 0; i < tables.length; i++) {
                        ...
                        if (rows.length > 5) {
                            ...
                            if (header.indexOf('序号') >= 0 && header.indexOf('药品名称') >= 0) {
                                // 突破性治疗表格：序号,受理号,药品名称,注册申请人,申请日期,...（无承办日期）
                                // 优先审评表格：序号,受理号,药品名称,注册申请人,承办日期,申请日期,...（有承办日期）
            ...
            if breakthrough_table_index < 0:
                logger.error("未找到突破性治疗表格")
                return drugs
            
            logger.info(f"确认突破性治疗表格索引: {breakthrough_table_index}")
            ...
            while total_collected < target_count:
                ...
                if tables_info.get('found'):
                    ...
                    for idx, item in enumerate(row_data):
                        ...
                        indication = get_drug_indication(page, "突破性治疗", idx + 1, breakthrough_table_index)
                ...
                # 翻页 - 使用正确的方法触发layui分页
                if total_collected < target_count:
                    # 获取当前页最后一条数据的序号，用于验证翻页成功
                    last_seq = row_data[-1].get('seq', '') if row_data else ''

                    # 使用正确的方法触发翻页：找到分页器并派发事件
                    clicked = page.evaluate("""
                        (tableIdx) => {
                            ...
                            if (!laypage) {
                                // 尝试另一种方法：查找所有分页器中包含下一页的
                                const allLaypages = document.querySelectorAll('.layui-laypage');
                                for (let lp of allLaypages) {
                                    const nextBtn = lp.querySelector('.layui-laypage-next');
                                ...
                                return { success: false, error: 'no enabled next button found' };
                            }
                            ...
                            // 检查按钮是否禁用
                            if (nextBtn.classList.contains('layui-disabled')) {
                            ...
                            nextBtn.dispatchEvent(event);
                            return { success: true, method: 'dispatchEvent from laypage' };
                        }
                    """, breakthrough_table_index)

                    if not clicked or not clicked.get('success'):
                        error_msg = clicked.get('error', 'unknown') if clicked else 'no result'
                        logger.info(f"翻页失败: {error_msg}，没有更多页了")
                        break

                    # 等待新页面加载完成 - 通过检测序号变化来确认翻页成功
                    max_wait = 20  # 最多等待20秒
                    wait_interval = 2
                    page_changed = False

                    for wait_time in range(wait_interval, max_wait + 1, wait_interval):
                        time.sleep(wait_interval)

                        # 检查页面是否已经加载了新数据
                        new_first_row = page.evaluate("""
                            (tableIdx) => {
                                const tables = document.querySelectorAll('table');
                                const targetTable = tables[tableIdx];
                        ...
                        # 如果序号发生变化，说明翻页成功
                        if new_first_row and new_first_row != last_seq:
                            page_changed = True
                            logger.info(f"翻页成功，新页面序号: {new_first_row}")
                            break

                    if not page_changed:
                        logger.warning(f"翻页后数据未变化，最后序号仍是: {last_seq}，继续等待...")
                        time.sleep(5)
...
def main():
    ...
    if breakthrough_drugs:
        # 读取优先审评数据
        priority_drugs = []
        if os.path.exists('data/cde_all_drugs_fixed_v2.csv'):
            with open('data/cde_all_drugs_fixed_v2.csv', 'r', encoding='utf-8-sig') as f:
        ...
        # 保存完整数据
        with open('data/cde_all_drugs_fixed_v3.csv', 'w', newline='', encoding='utf-8-sig') as f:
        ...
        logger.info("完整数据已保存到: data/cde_all_drugs_fixed_v3.csv")
        
        # 筛选抗肿瘤药物 - 同时处理布尔值和字符串两种情况
        def is_anti_cancer(val):
        ...
        anticancer_drugs = [d for d in all_drugs if is_anti_cancer(d.get('是否抗肿瘤', False))]
        
        with open('data/cde_anticancer_drugs_fixed_v3.csv', 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.DictWriter(f, fieldnames=[
                '名单类型', '序号', '药物名称', '受理号', '申请人',
                '申请日期', '拟定适应症', '是否抗肿瘤'
            ])
